async function checkGardenWater() {
  ...
  try {
    ...
    if (zone3.length > 0 || zone4.length > 0 || zone5.length > 0) {
      ...
      this.logger.trace(
        `${this._traceStack()} - House plants do not need watering`,
      );
    }

    // Garden zones
    if (zone1.length > 0 || zone2.length > 0) {
      this.logger.trace(`${this._traceStack()} - Garden needs watering`);
      ...
      this.logger.trace(`${this._traceStack()} - It's not going to rain`);
 ...
 */
async function setupSchedules(firstRun) {
  ...
  if (!firstRun) {
    this.logger.trace(`${this._traceStack()} - Scan for devices`);
    this._scanDevices();
  }
  return true;
}
