...
async function checkGardenWater() {
  ...
  const deviceTokens = [];
  const wateringDuration = 10; // in minute(s)
  ...
  try {
    ...
    const AlfredControllerService = await serviceHelper.vaultSecret(
      process.env.ENVIRONMENT,
      'AlfredControllerService',
    );
    ...
    const LinkTapLinkerID = await serviceHelper.vaultSecret(
      process.env.ENVIRONMENT,
      'LinkTapLinkerID',
    );
    ...
    serviceHelper.log(
      'info',
      'Turning on the garden watering system',
    );
    ...
    const IOSNotificationKeyID = await serviceHelper.vaultSecret(
      process.env.ENVIRONMENT,
      'IOSNotificationKeyID',
    );
    const IOSNotificationTeamID = await serviceHelper.vaultSecret(
      process.env.ENVIRONMENT,
      'IOSNotificationTeamID',
    );
    const IOSPushKey = await serviceHelper.vaultSecret(
      process.env.ENVIRONMENT,
      'IOSPushKey',
    );
    ...
    if (result.sent.length > 0) {
      serviceHelper.log(
        'info',
        'Water garden push notification sent',
      );
    } else {
      serviceHelper.log(
        'error',
