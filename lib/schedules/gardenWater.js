async function checkGardenWater() {
  ...
  try {
    ...
    if (zone1.length > 0 || zone2.length > 0) {
      ...
      const LinkTapUser = await this._getVaultSecret.call(
        this,
        process.env.ENVIRONMENT,
        'LinkTapUser',
      );
      const LinkTapKey = await this._getVaultSecret.call(
        this,
        process.env.ENVIRONMENT,
        'LinkTapKey',
      );
...
async function setupSchedule(data) {
  ...
  this.logger.trace(
    `${this._traceStack()} - Add water schedule to base class schedules`,
  );
  this._addSchedule(date, 'Plant water check', checkGardenWater);
  return true;
}
