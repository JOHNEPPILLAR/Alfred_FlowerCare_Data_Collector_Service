...
async function _sensors(req, res, next) {
  ...
  const { gardenSensorAddress, durationSpan } = req.params;
  if (
  ...
  try {
    switch (durationSpan.toLowerCase()) {
      case 'year':
        timeBucket = '6 hours';
        interval = '1 year';
        break;
    ...
    if (results.rowCount === 0) {
      this.logger.error(`${this._traceStack()} - No data to return`);
    }
...
async function _sensorsZone(req, res, next) {
  ...
  try {
    ...
    const returnData = [];
    this.logger.trace(`${this._traceStack()} - Get readings for each zone`);
    await Promise.all(
