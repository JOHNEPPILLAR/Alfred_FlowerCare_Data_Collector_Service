...
async function _getFlowerCareDevices() {
  try {
    ...
    if (!this.unStableBLECounter) this.unStableBLECounter = 0;
    this.devicesInError = 0;
    this.devicesActive = 0;
    ...
    await Promise.all(
      devicesToScan.map(async (deviceID) => {
        ...
        if (device) {
          ...
          this.devicesActive += 1;
          await _getFlowerCareData.call(this, device);
        } else {
          this.logger.info(`Device ${deviceID} was not found`);
        }
        return true;
      }),
    );

    if (this.devicesInError === 0) {
      this.unStableBLECounter = 0;
    } else if (this.devicesInError === this.devicesActive) {
      this.unStableBLECounter += 1;
    }
    if (this.unStableBLECounter === 5) {
      const message = `👾 Too many BLE timeouts on ${process.env.SERVER_NAME}`;
      this.logger.info(message);
      this.unStableBLECounter = 0; // Reset counter
      this._sendPushNotification.call(this, message);
    }

    const poolingInterval = 15 * 60 * 1000; // 15 minutes
    setTimeout(async () => {
      await _getFlowerCareDevices.call(this);
