...
const devicePollingIntival = 2 * 60 * 60 * 1000; // 2 hrs
...
async function processDevice(peripheral) {
  try {
    ...
    debug(`Connecting to peripheral: ${peripheral.location} - ${peripheral.plant} (${deviceAddress})`);
    await peripheral.peripheral.connectAsync();

    debug(`Getting data from peripheral: ${peripheral.location} - ${peripheral.plant} (${deviceAddress})`);
    ...
    debug(`Getting firmware data from: ${peripheral.location} - ${peripheral.plant} (${deviceAddress})`);
    ...
    debug(`Getting sensor data from: ${peripheral.location} - ${peripheral.plant} (${deviceAddress})`);
    ...
    debug(`Disconnect peripheral: ${peripheral.location} - ${peripheral.plant} (${deviceAddress})`);
...
async function processDevices() {
  // Clear existing data collection pooling intival
...
async function _getFlowerCareDevices(nobleSetup) {
  ...
  try {
    ...
    debug(`Found ${this.devicesInZone.length} devices in zone ${zone}`);
  } catch (err) {
    this.logger.error(`${this._traceStack()} - ${err.message}`);
  } finally {
    debug(`Close DB connection`);
  ...
  if (nobleSetup) {
    ...
    noble.on('discover', async (peripheral) => {
      try {
        ...
        if (deviceAddress === '') {
          debug(`Device does not have an address: ${peripheral.id}`);
          return;
        }
        ...
        if (newDevice.length === 0) {
          debug(`Device not listed in zone: ${deviceAddress}`);
          return;
        }

        // Check if peripheral already assigned
        if (typeof newDevice[0].peripheral !== 'undefined') {
          debug(`Existing device found: ${newDevice[0].location} - ${newDevice[0].plant} (${deviceAddress})`);
          return;
        }
