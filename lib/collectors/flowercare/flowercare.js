...
async function getFlowerCareData(device) {
  ...
  try {
    this.logger.trace(
      `${this._traceStack()} - Get sensor data from: ${device.address}`,
    );
...
async function getFlowerCareDataWrapper(device) {
  return new Promise((resolve) => {
    const timeout = setTimeout(() => {
      this.logger.error(
        `${this._traceStack()} - Force getting sensor data timeout on device: ${
          device.address
        }`,
      );
      resolve();
    }, 120000); // Force timeout after 2 minute

    // eslint-disable-next-line no-unused-expressions
    async () => {
      await getFlowerCareData.call(this, device);
  ...
  try {
    ...
    devicesToScan.map(async (deviceID) => {
      ...
      if (device) {
        ...
        await getFlowerCareDataWrapper.call(this, device);
      } else {
        this.logger.trace(
