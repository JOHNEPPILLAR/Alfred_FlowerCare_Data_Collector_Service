...
async function startScanning() {
  ...
  setTimeout(async () => {
    try {
      ...
      this.logger.error(
        `${this._traceStack()} - Not able to find all device(s)`,
      );

      if (scanCounter === 3) {
        scanCounter = 0; // Reset counter
        this.logger.error(
          `${this._traceStack()} - Max discovery retry hit. Processing what was found`,
        );
...
async function processDevice(peripheral) {
  ...
  const sensorJSON = {
    ...
    threshold_moisture: this.devices[deviceAddress].threshold_moisture,
    threshold_fertilizer: this.devices[deviceAddress].threshold_fertilizer,
  };
...
async function _getFlowerCareDevices() {
  ...
  noble.on('discover', (peripheral) => {
    try {
      ...
      let validDevice = this.devicesFound[deviceAddress];
      if (validDevice) {
        this.logger.debug(
      ...
      validDevice = this.devices[deviceAddress];
      if (!validDevice) {
        this.logger.debug(
      ...
      this.logger.info(
        `Adding device: ${deviceInfo.location} - ${deviceInfo.plant} (${deviceAddress})`,
      );
