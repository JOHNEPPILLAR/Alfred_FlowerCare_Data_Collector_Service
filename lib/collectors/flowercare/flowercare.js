...
async function processDevice(peripheral) {
  try {
    ...
    const characteristics =
      await peripheral.peripheral.discoverAllServicesAndCharacteristicsAsync();
    const sensorJSON = {
      time: new Date(),
...
async function discoverDevices() {
  ...
  noble.on('discover', async (peripheral) => {
    ...
    if (currentDevice.length === 0) {
      this.logger.info(`Found device not assigned to zone: ${peripheral.id}`);
    ...
    this.logger.info(
      `Found peripheral: ${this.devicesInZone[objIndex].location} - ${this.devicesInZone[objIndex].plant} (${peripheral.id})`,
    );
