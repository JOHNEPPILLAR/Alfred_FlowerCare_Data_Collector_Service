...
async function setupServer() {
  ...
  service.restifyServer.get('/scan', (req, res, next) =>
    service._scanDevices(req, res, next),
  );
  service.logger.trace(`${service._traceStack()} - Added '/scan' api`);
  ...
  service.restifyServer.get('/sensors/:zone', (req, res, next) =>
    service._sensorsZone(req, res, next),
  );
  service.logger.trace(`${service._traceStack()} - Added '/sensors/:zone' api`);

  service.restifyServer.get('/current', (req, res, next) =>
    service._current(req, res, next),
  );
  service.logger.trace(`${service._traceStack()} - Added '/current' api`);
  ...
  if (process.env.MOCK === 'true') {
    ...
    service._getFlowerCareDevices(); // Collect Flowercare device data
    if (process.env.NO_SCHEDULE === 'true') {
      service.logger.info('Collect data only and do not set any schedules');
    } else {
      service.setupSchedules(); // Setup garden schedules
    }
  }
}
setupServer();
